...
import { Spirit } from "https://edankwan.com/experiments/the-spirit/spirit.js";
...
function init() {
  ...
  var motionMultiplier = postprocessingGui
    ...
    .listen();
  var motionQuality = postprocessingGui
...
function _render(dt, newTime) {
  ...
  for (var i = 0, len = _footerItems.length; i < len; i++) {
    ...
    _footerItems[i].style.transform =
      "translate3d(0," + (1 - Math.pow(ratio, 3)) * 50 + "px,0)";
  }

  ratio = math.unLerp(0.5, 0.6, _initAnimation);
  if (!settings.isMobile) {
    _instruction.style.display = ratio ? "block" : "none";
    _instruction.style.transform =
      "translate3d(0," + (1 - ratio * ratio) * 50 + "px,0)";
  }
